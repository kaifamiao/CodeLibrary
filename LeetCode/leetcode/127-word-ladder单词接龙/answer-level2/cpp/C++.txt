...
int BFS_graph(std::string &beginWord, std::string &endWord, std::map<std::string,std::vector<std::string>> &graph){
    ...
    while(!Q.empty()){
        ...
        const std::vector<std::string> &nei***ors = graph[node];//取出全部临街点遍历
        for(int i = 0; i < nei***ors.size(); i++){
            if(visit.find(nei***ors[i]) == visit.end()){
                Q.push(std::make_pair(nei***ors[i],step+1));
                visit.insert(nei***ors[i]);//标记nei***ors已经进入队列了
            }
        }
    }
    return 0;//否则返回没找到

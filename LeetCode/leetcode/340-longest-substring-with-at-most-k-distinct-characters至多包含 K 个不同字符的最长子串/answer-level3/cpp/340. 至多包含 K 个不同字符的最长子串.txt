...
3、value值得作用：map中有超过2个不同的字符时，自左至右移动左指针，直到子串中的不同字符少于k+1个
...
public:
    int lengthOfLongestSubstringKDistinct(string s, int k) {
        int maxlen_substr = 0;//含有最多k个不同字符的最长子串的长度。
        int length = s.size();
        if(0 == length || k == 0){
        ...
        int num_of_element = 0;//子串中不同字符的个数，如果该值大于k说明子串中不同字符的个数已经超过k个了，需要移动左指针，直到子串中的不同字符个数＜k+1
        ...
        for(int i = 0;i < length;i++){//从0来时遍历整个字符串
            ...
            if(num_of_element > k){//子串中不同字符的个数超过k个时的处理逻辑

...
在判断删除后是否符合要求时，记得要保存遍历的节点数量，仅当成功遍历的节点数量等于N时，才算是符合要求。假若你删除的恰好是局部环外的某条边，那么依照我的代码，queue最后也会变成空，但此时显然不符合题意，所以要加上遍历的节点的数量，这个和你判断是否不为环的逻辑可能有差异。

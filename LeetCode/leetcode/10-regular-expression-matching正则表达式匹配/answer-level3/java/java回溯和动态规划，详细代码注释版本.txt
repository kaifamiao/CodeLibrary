...
但是，"\*"这个特殊字符需要特殊处理，当`p的第i个元素的下一个元素是星号时`会有两种情况：
...
2. 这个boolean类型的`dp数组的大小应该是dp[s.length+1][p.length+1]`，因为我们不仅仅要分别取出s和p的所有元素，还要表示分别取s和p的0个元素时候(都为空)的情况。
...
然后填写数组的其余部分，这个过程中如果`p.charAt(j)=='*\'`依然是遵循上题中的两种情况；否则就`判断两个字符串的i和j号字符是否相等，相等则分别减除当前字符继续判断，不相等则直接等于false`。
...
有了第一个思路总结的"经验"之后，这道题逻辑上不难理解，但是细节上尤其各种下标值非常的恶心。

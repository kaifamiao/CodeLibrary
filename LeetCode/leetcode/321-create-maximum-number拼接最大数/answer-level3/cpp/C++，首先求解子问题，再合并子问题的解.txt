...
假设最大子序列中，包含来自nums1的`i`个元素和来自nums2的`k-i`个元素，

那么可以通过反证法证明，来自nums1的`i`个元素，构成了nums1的长度为`i`的最大子序列；来自nums2的`k-i`个元素，构成了nums2的长度为`k-i`的最大子序列。

因此，可以首先分别求出nums1中长度为`i`的最大子序列，和nums2中长度为`k-i`的最大子序列，然后求它们归并起来的最大子序列。

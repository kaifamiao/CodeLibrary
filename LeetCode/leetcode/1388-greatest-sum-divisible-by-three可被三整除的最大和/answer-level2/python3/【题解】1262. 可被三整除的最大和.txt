...
- 时间复杂度：$O(N \log N)$，其中 $N$ 是数组 `nums` 的长度。在平均情况下，数组 `b` 和 `c` 的长度规模与 `nums` 相当，因此可以用 $O(N \log N)$ 近似将 `b` 和 `c` 进行排序的时间复杂度。
...
- 时间复杂度：$O(N \log N)$，其中 $N$ 是数组 `nums` 的长度。也可以不用排序，将时间复杂度优化至 $O(N)$。
